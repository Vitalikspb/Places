...
//        https://api.openweathermap.org/data/2.5/onecall?lat=59.939634&lon=30.3104843&exclude=minutely,hourly&units=metric&appid=8a8dd7602db62946ca2c5ab51405a786&lang=ru
        ...
        URLSession.shared.dataTask(with: wheatherURL) { (data, response, error) in
            ...
            do {
                ...
                self.loadIconFromApi(with: forecast.current.weather.first!.icon) { image in
                    currentWeatherOfSevenDays = CurrentWeatherOfSevenDays(todayTemp: forecast.current.temp,
                                                                              imageWeather: image!,
                ...
                forecast.daily.forEach { item in
                    self.loadIconFromApi(with: item.weather.first!.icon) { image in
                    weatherSevenDaysArray = [WeatherSevenDays(dayOfWeek: item.dt,
    ...
    func loadIconFromApi(with iconCode: String?, completion: @escaping(UIImage?)->Void) {
